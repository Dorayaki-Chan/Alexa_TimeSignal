...
```typescript
{
  wakeUp: { enabled: true, defaultTime: '07:00', weekendEnabled: false, holidayEnabled: false },
...
| `shouldSkipMorning(config)` | `shouldSkipToday` + 週末チェック + 祝日チェック |
...
interface AppConfig {
    wakeUp: {
        enabled: boolean;         // 起床シーケンス有効化
        defaultTime: string;      // HH:MM 形式（デフォルト: "07:00"）
        weekendEnabled: boolean;  // 週末実行（デフォルト: false）
        holidayEnabled: boolean;  // 祝日実行（デフォルト: false）
    };
    timeSignal: {
...
| `input_datetime.alexa_kisho_time` | Time | 07:00 |
