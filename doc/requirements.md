...
| 週末スキップ | 土日は自動的にスキップ |
| 祝日スキップ | `@holiday-jp/holiday_jp` による日本祝日判定で自動スキップ |
| 次回オーバーライド | HAから特定の日時を1回だけ指定可能。土日祝日でも指定日・指定時刻に実行される。食事ラッパ実行後に自動クリアされ、HA側のトグルも自動OFFになる |
...
| nextWakeUp | enabled（次回オーバーライド有効化） | `false` |
| nextWakeUp | date（次回起床日） | `""` |
| nextWakeUp | time（次回起床時刻） | `"07:00"` |
| events | イベント時報の配列 | `[]` |
...
| `input_boolean.alexa_next_wakeup_enabled` | Boolean | 次回起床オーバーライド有効化 |
| `input_datetime.alexa_next_wakeup_datetime` | DateTime | 次回起床日時 |
...
**連携方式**
- RESTセンサー: `/api/status` を60秒間隔でポーリングし、次の時報予定とオーバーライド状態を表示
- RESTコマンド: 入力ヘルパー変更時にオートメーションで `/api/config` へPUT送信
- オーバーライド自動OFF: バックエンドでオーバーライドがクリアされると、RESTセンサー経由でHA側のトグルも自動OFFになる
